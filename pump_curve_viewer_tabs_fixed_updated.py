...
def plot_lines(df, model_col, x_col, y_col, selected_models, source=None, linestyle=None):
    ...
    for model in selected_models:
        ...
        label = model.replace("-통출양정", "").replace("-추동력", "")
...
def process_and_plot(sheet_name, point_only=False, catalog_style=False, ai_mode=False, total_mode=False, tab_id="default"):
    ...
    try:
        ...
        if ai_mode or total_mode:
            st.warning("Reference 데이터를 불러오지 않을 수 있습니다.")
        return
    ...
    if df is not None:
        ...
        y2_col = get_best_match_column(df, ["추동력"])
...
if uploaded_file:
    tabs = st.tabs(["Total", "Reference", "Catalog", "Deviation", "AI 배우기"])

    with tabs[0]:
        st.subheader("📊 Total - 통합 고선 분석")
